...
- **라이브 영상 (다중 카메라, 다중 포맷)**: RTSP·HLS·RTMP·DASH를 URL에서 자동 판별해 `createVideo*()` + `registerVideoUrlListener()` 로 네이티브 스트리밍(클라우드/트랜스코딩 없음). 한 기기에 카메라 1~8대.
...
- 트리거: `doorbell_rang`(초인종 눌림), `motion_detected`(서성임 감지), `lock_locked`(도어락 잠김), `lock_unlocked`(도어락 해제)
- 조건: `lock_is_locked`(잠겨 있으면), `motion_is_active`(서성임 감지 중), `doorbell_is_ringing`(초인종 울리는 중)
- 액션: `lock_door`(문 잠그기), `unlock_door`(문 열기)
