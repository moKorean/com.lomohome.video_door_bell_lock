...
/**
 * Resolve the still-image (JPEG) snapshot URI of an ONVIF camera.
 * @returns {Promise<string>} the http(s):// snapshot URL
 */
function getSnapshotUri({ ip, onvifPort = 80, username = '', password = '' }, timeout = 8000) {
  return new Promise((resolve, reject) => {
    const onvif = loadOnvif();
    if (!onvif || !onvif.Cam) return reject(new Error('ONVIF library not available'));
    // eslint-disable-next-line no-new
...
module.exports = {
  onvifProbe, getRtspUri, getSnapshotUri, scanPorts,
};
